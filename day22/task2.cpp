...
int main()
{
    ...
    for(brick & b : bricks)
    {
        std::vector<brick*> tofall;
        std::vector<bool> fallen(bricks.size(), false);
        tofall.push_back(&b);
        int n_fallen = 0;
        while(!tofall.empty())
        {
            brick * br = tofall.back();
            tofall.pop_back();
            size_t idx = br - bricks.data();
            if(!fallen[idx])
            {
                n_fallen++;
                fallen[idx] = true;
                for(brick * bnext : br->supporting)
                {
                    if(std::all_of(bnext->supported_by.begin(), bnext->supported_by.end(), [&](const brick * bs){ return fallen[bs - bricks.data()]; }))
                    {
                        tofall.push_back(bnext);
                    }
                }
            }
        }
